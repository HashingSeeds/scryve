...
Identify the changed behavior and exercise the actual exported functions. Verify the resulting records or query output, not just that the call returned. For ownership changes, use the permitted caller, a different user, and an unauthenticated caller. For migrations or outbox recovery, test interrupted work and retries instead of unrelated auth cases.
