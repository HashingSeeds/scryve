...
Count's Maestro flows exercise an installed development build by bundle/package ID
...
3. Install a Count development build (`pnpm run ios` or `pnpm run android`). Expo Go is not
   sufficient for these flows because they launch Count by its own app ID.
